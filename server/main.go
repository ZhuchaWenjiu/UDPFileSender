...
func handleClient(n int, addr *net.UDPAddr, buf []byte, err error, conn *net.UDPConn) {
	...
	if fileReq.Start == math.MaxInt64 {
		...
		file.Seek(int64(fileReq.Start), 0)                 // 相对于文件开头，偏移 fileReq.Start的位置
		content := make([]byte, fileReq.End-fileReq.Start) // 内存分配，数据类型是 字节数组，大小是 end - start
		_, err := file.Read(content)                       // 因为前面file.Seek了，所以可以直接Read
		if err != nil {
	...
	resp, err := json.Marshal(fileResp) // 将 fileResp 编码成 json
	if err != nil {
